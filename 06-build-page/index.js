        ...
        data.forEach(el => {
            fs.stat(path.join(__dirname, "styles", el), (err, stat) => {
                ...
                if(stat.isFile() && el.split(".")[1] === "css") {
                    fs.readFile(path.join(__dirname, "styles", el), function (err, content) {
                        if(err) {
                            throw err;
                        } else {
                            const dataBuffer = Buffer.from(content);
                            fs.appendFile(path.join(__dirname, "project-dist", "style.css"), dataBuffer, (err) => {
                                if(err) {
                                    throw  err;
                                }
                            });
    ...
    function copyDir(filesPath, copiedPath) {
        fs.readdir(filesPath, { withFileTypes: true }, (err, data) => {
            if (err) {
                ...
                data.forEach(el => {
                    if (el.isFile()) {
                        fs.copyFile(path.join(filesPath, el.name), path.join(copiedPath, el.name), err => {
                            if (err) throw err;
                        });
                    } else if (el.isDirectory()) {
                        fs.mkdir(copiedPath+"\\"+el.name,{recursive:true}, err => {
                            if (err) throw err;
                        });
                        copyDir(path.join(filesPath, el.name), path.join(copiedPath, el.name));
                    }
                });
            }
        });
